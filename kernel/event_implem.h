...
void __pi_evt_signal_wait(pi_evt_t *event, pi_thread_t *current_thread);
...
{
    int irq = pi_irq_lock();
    pi_evt_signal_wait_unsafe(event);
    pi_irq_unlock(irq);
}
...
ALWAYS_INLINE void pi_evt_signal_wait_unsafe(pi_evt_t *event)
{
    if (event->callback)
    {
        __pi_evt_signal_wait(event, __pi_thread_current);
    }
}
...
{
    pi_evt_cb(event, __pi_evt_handle_signal);
...
{
    pi_evt_cb(event, __pi_evt_push_task);
